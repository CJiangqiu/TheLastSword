...
public class AbsoluteDestructionDamageSource extends DamageSource {
    ...
    public static boolean applyAbsoluteDestructionIntelligently(LivingEntity entity, DamageSource damageSource, float damageAmount) {
        // 如果实体已被禁疗，使用禁疗管理器的记录值作为当前血量
        float originalHealth;
        if (AttackManager.isHealNegated(entity)) {
            originalHealth = AttackManager.getLockedHealth(entity);
        } else {
        ...
        // 步骤3: 检查实际vs预期生命值（同样使用禁疗管理器的记录值）
        float actualHealth;
        if (AttackManager.isHealNegated(entity)) {
            actualHealth = AttackManager.getLockedHealth(entity);
        } else {
            actualHealth = entity.getHealth();
        }
        ...
        // 步骤5: 设置禁疗状态
        AttackManager.setHealNegation(entity, expectedHealth);
        return true;
    }
